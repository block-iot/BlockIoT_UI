...
function get_formatted_date(ms_date) {
  ...
  if (js_date.getHours() < 20) {
    display_date =
      " " +
      get2D(js_date.getMonth() + 1) +
      "/" +
      get2D(js_date.getDate()) +
      " " +
      get2D(js_date.getHours() + 4) +
      ":" +
      get2D(js_date.getMinutes());
  } else {
    display_date =
      " " +
      get2D(js_date.getMonth() + 1) +
      "/" +
      get2D(js_date.getDate() + 1) +
      " " +
      get2D(js_date.getHours() - 20) +
      ":" +
      get2D(js_date.getMinutes());
  }
...
function updateExtremes() {
  //update time axes
  $("#selectedTimes").text(
    get_formatted_date(selectedMin) + " to " + get_formatted_date(selectedMax)
  );
  try {
    ...
    $("#lab-time11")
      .highcharts()
      .xAxis[0].setExtremes(selectedMin, selectedMax);
    $("#lab-time12")
      .highcharts()
      .xAxis[0].setExtremes(selectedMin, selectedMax);
    $("#lab-time13")
  ...
  for (var i = 0; i < chartsContainers.length; i++) {
    ...
    for (q = 0; q < currData.length; q++) {
      // Lookes for data point with currently selected time range
    ...
    if (notPoints) {
      // Hides charts when no points in selected range
...
function add_vertical_point(point_time, from_note) {
  if (typeof from_note !== "undefined") {
    ...
    var st_display =
      " " +
      get2D(band_start.getMonth() + 1) +
      "/" +
      get2D(band_start.getDate() + 1);
...
function add_observation_chart(
  obs_id,
  observation_details,
  variable_details,
  panel_1_groups
...
function get_lab_chart(
  ...
  for (var i = chart_data[0].data.length - 1; i >= 0; i--) {
    if (chart_data[0].data[i][0] <= displayed_max_t) {
      ...
      if (isDiscrete) {
        most_recent_val =
          observation_details.discrete_nominal_to_yIndex[most_recent_val];
      }
      break;
    }
  }

  if (variable_details.display_name.includes("Smart")) {
    // create and render chart //
    if (variable_details.display_name.includes("Glucose")) {
      var currChart = new Highcharts.Chart({
        ...
        credits: {
          text:
            ...
            most_recent_val +
            '</p><br><p style="font-size:8px">' +
            observation_details.units +
            "</p>",
          href: "",
          zIndex: 0,
          ...
          formatter: function () {
            if (this.series.name === "numeric_values") {
              return (
                ...
                Highcharts.dateFormat("%m/%d", this.x) +
                "</b> " +
                this.y +
                "</p>"
              );
      ...
      tooltip: {
        ...
        formatter: function () {
          if (this.series.name === "numeric_values") {
            ...
            index = this.point.series.xData.indexOf(this.point.x);
            return (
              chart_data[1].data[index][1] + "/" + chart_data[0].data[index][1]
            );
          } else {
            return (
              '<p style="font-size:12px">' +
...
function get_med_chart(chart_container_id, medication_details, med_details) {
  var chart_height =
    100 + 15 * Math.floor(med_details.display_name.length / 30);
  ...
  if (med_details.display_name.includes("(")) {
    // Create chart
    var currChart = new Highcharts.Chart({
      chart: {
        renderTo: chart_container_id,
...
function getchartTS(id, case_details, time_step = 0) {
  console.log(case_details[0]["min_t"]);
  if (case_details[0]["min_t"] == 1662091200001) {
    $(id).highcharts("StockChart", {
      chart: {
        height: 35,
        spacingLeft: 5,
        ...
        events: {
          load: function () {
            var range = this.xAxis[0].getExtremes();
            this.xAxis[0].setExtremes(
              Math.max(range.min, 1669352400000),
              range.max
            );
          },
        }, // On load update selected range // 2.5 days
      ...
      chart: {
        ...
        events: {
          load: function () {
            var range = this.xAxis[0].getExtremes();
            this.xAxis[0].setExtremes(
              Math.max(range.min, range.max - 21600000000000),
              range.max
            );
          },
        }, // On load update selected range // 2.5 days
