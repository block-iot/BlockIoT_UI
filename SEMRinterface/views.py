...
def select_case(request, study_id, user_id):
    ...
    template = loader.get_template(
        'SEMRinterface/case_selection_screen_part1.html')
    if "Part1" in user_id:

        template = loader.get_template('SEMRinterface/case_selection_screen_part1.html')
    if "Part2" in user_id:
        template = loader.get_template(
            'SEMRinterface/case_selection_screen_part2.html')
    context_dict = {
...
def case_viewer(request, study_id, user_id, case_id, time_step=0):
    ...
    if "Hosp" in case_id:
        ...
        template = loader.get_template(
            'SEMRinterface/case_viewer_outpatient.html')
    context_dict = {
        'case_id': case_id,
        'user_id': user_id,
        'study_id': study_id,
